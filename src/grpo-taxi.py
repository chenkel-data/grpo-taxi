...
from src.utils import get_checkpoint, step, load_dataset, decode, LOCS, set_up_logger
...
class TaxiGRPOTrainer:
    ...
    def action_reward(self, completions, **kwargs):
        ...
        state_txts = [state_txt[1]["content"]
                      for state_txt in kwargs["prompts"]]
        states = kwargs["state"]

        rewards = []
        for idx, action in enumerate(actions):
            state_desc = state_txts[idx].split(
                "### Current State:")[-1].split(" ### Output:")[0].strip()
            state = states[idx]
            ...
            self.logger.debug(
                f"State positions: {taxi_row, taxi_col, pass_idx_current, dest_idx}")
            ...
            pass_loc = LOCS[pass_idx_current] if pass_idx_current < 4 else (
                taxi_row, taxi_col)
            positions = (taxi_row, taxi_col), pass_loc, LOCS[dest_idx]
            self.logger.debug(f"Positions: {positions}")

            # Take action in environment
            next_state, env_reward, done, _, _ = step(
                self.env, state, predicted_action)

            # Decode next state
            next_state_positions = self._get_positions_from_state(next_state)
            self.logger.debug(f"Next state: {next_state_positions}")
            ...
            self.logger.debug(
                f"Action reward: {action_reward}, Env reward: {env_reward}")
    ...
    def _calculate_pickup_reward(self, action, positions, next_positions):
        ...
        self.logger.debug(
            "Passenger not picked up yet. Get distance of taxi and passenger")
        ...
        self.logger.debug(
            f"Delta current: ({delta_y_current}, {delta_x_current})")
    ...
    def _calculate_dropoff_reward(self, positions, next_positions):
        ...
        self.logger.debug(
            "Passenger picked up. Get distance of taxi and destination.")
    ...
    def load_data(self):
        ...
        train_dataset, test_dataset = load_dataset(file_path=DATA_PATH)
